...
//we are taking coeff_matrix in fixed point
double num[3][4] = {0.0216,0.0216,0.0139,0.0139,0,0,0,0,-0.0216,-0.0216,-0.0139,-0.0139};
double den[2][4] = {-1.9785,-1.9900,-1.9685,-1.9762,0.9885,0.9934,0.9757,0.9807};
int16_t intnum[3][4], intden[2][4];
...
const unsigned long MAX_RECORDING_LENGTH_MS = 2000;
	...
	for(int i = 0; i < 3;i++)
		{
			for(int j =0 ;j<4; j++)
				{
					intnum[i][j] = static_cast<int16_t>(num[i][j]*pow(2,14));
				}
		}    
	...
	for(int i = 0; i< 2;i++)
		{
			for(int j = 0;j<4;j++)
				{
					intden[i][j] = static_cast<int16_t>(den[i][j]*pow(2,14));
				}
		}
}
...
void loop() {
    ...
    case STATE_CONNECT:
        ...
        if (client.connect(serverAddr, serverPort) && client1.connect(serverAddr,serverPort2)) {
            // Connected
    ...
    case STATE_RUNNING:
        ...
        if (sendBuf != NULL) {
            ...
            int count = client.write((uint8_t *)sendBuf, SAMPLE_BUF_SIZE / 2); //original      
			...
			int len_buffer =SAMPLE_BUF_SIZE/4;
			uint16_t *result = sendBuf;
			std::vector<int16_t> xs (len_buffer +2 );
			
			xs[0] = 0;
			xs[1] = 0;
          
			int16_t ys[len_buffer+2];
            ...
            for (unsigned int j = 0, i =2; j < SAMPLE_BUF_SIZE/4; j++, i++){
                xs[i] = static_cast<int16_t>(sendBuf[j] - (1<<15));
                ys[i] = 0;
            }
          
        
			for (unsigned int i = 0; i < 4; i++) {
				for (unsigned int j = 2; j < len_buffer+2; j++) {
           
					ff2 = static_cast<int32_t>(intnum[0][i]) * (xs[j]);
					ff2+= static_cast<int32_t>(intnum[1][i]) * (xs[j-1]);
					ff2+= static_cast<int32_t>(intnum[2][i]) * (xs[j-2]);
              
					fb2 = static_cast<int32_t>(intden[0][i]) * (ys[j-1]);
					fb2 += static_cast<int32_t>(intden[1][i]) * (ys[j-2]);
           
					ys[j] = static_cast<int16_t>((ff2 - fb2)/(pow(2.0, 14)));
				}
				for (unsigned int k = 0; k < (len_buffer+2); k++) {
					xs[k] = ys[k];
				}
			}
        
            int16_t *filtered_data = ys+2;
            
            int16_t temp[512];
            
            for(int l=0;l<512;l++)
            {
                temp[l] = abs(filtered_data[l]) / 10;
			...
			//client.write takes const  char ** so send bytes
            client1.write((uint8_t *)filtered_data, SAMPLE_BUF_SIZE / 2);
...
//        samples[2048] = {0};
        state = STATE_WAITING;
        Particle.publish("abc",String(test_energy));
        if(test_energy>=150)
            {
                RGB.control(true);                  
                RGB.color(255,0,0);
